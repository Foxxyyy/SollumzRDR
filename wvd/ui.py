...
class SOLLUMZ_PT_DRAWABLE_PANEL(bpy.types.Panel):
    ...
    def poll(cls, context):
        return context.active_object is not None and context.active_object.sollum_type == SollumType.VISUAL_DICTIONARY
...
class SOLLUMZ_PT_DRAWABLE_MODEL_PANEL(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        sollumz_parent = find_sollumz_parent(
            obj, parent_type=SollumType.VISUAL_DICTIONARY)
...
class SOLLUMZ_UL_SHADER_MATERIALS_LIST(bpy.types.UIList):
    ...
    def draw_item(
        ...
        name = rdr1_shadermats[item.index].ui_name
...
class SOLLUMZ_PT_CREATE_DRAWABLE_PANEL(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(ydr_ops.SOLLUMZ_OT_convert_to_drawable_model.bl_idname, icon="MESH_DATA")

        row = layout.row()
        row.operator(ydr_ops.SOLLUMZ_OT_convert_to_drawable.bl_idname, icon="OUTLINER_OB_MESH")
        ...
        row.operator(ydr_ops.SOLLUMZ_OT_create_drawable.bl_idname, icon="OUTLINER_OB_MESH")
