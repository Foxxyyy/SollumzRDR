...
def export_wvd(drawable_obj: bpy.types.Object, filepath: str) -> bool:
    export_settings = get_export_settings()
    drawable_xml = create_wvd_xml(drawable_obj, export_settings.exclude_skeleton)  
    write_embedded_textures(drawable_obj, filepath)
    drawable_xml.write_xml(filepath)
    return True

def create_wvd_xml(wvd_obj: bpy.types.Object, exclude_skeleton: bool = False):
    wvd_xml = RDR1VisualDictionary()
    wvd_armature = find_wvd_armature(wvd_obj) if wvd_obj.type != "ARMATURE" else wvd_obj

    for child in wvd_obj.children:
...
def create_model_xml(model_obj: bpy.types.Object, lod_level: LODLevel, materials: list[bpy.types.Material], bones: Optional[list[bpy.types.Bone]] = None, transforms_to_apply: Optional[Matrix] = None):
    ...
    geometries = create_geometries_xml(mesh_eval, materials, bones, model_obj.vertex_groups)
