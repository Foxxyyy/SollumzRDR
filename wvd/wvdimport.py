...
from ..cwxml.drawable import WVD, BoneLimit, Joints, LodList, RDR1VisualDictionary, Shader, ShaderGroup, Drawable, Bone, Skeleton, RotationLimit, DrawableModel
...
def import_wvd(filepath: str):
    import_settings = get_import_settings()

    name = get_filename(filepath)
    wvd_xml = WVD.from_xml_file(filepath)

    global current_game
    current_game = wvd_xml.game

    if import_settings.import_as_asset:
        return create_drawable_as_asset(wvd_xml, name, filepath)

    return create_wvd_obj(wvd_xml, filepath)

def create_wvd_obj(wvd_xml: RDR1VisualDictionary, filepath: str):
    name = get_filename(filepath)
    dict_obj = create_empty_object(SollumType.VISUAL_DICTIONARY, name, current_game)

    wvd_xml_list = wvd_xml
    if current_game == SollumzGame.RDR1:
        wvd_xml_list = wvd_xml.drawables

    for drawable_xml in wvd_xml_list:
        drawable_obj = create_drawable_obj(drawable_xml, filepath, external_bones=None, game=current_game)
        drawable_obj.parent = dict_obj

    return dict_obj

def create_drawable_obj(drawable_xml: Drawable, filepath: str, name: Optional[str] = None, split_by_group: bool = False, external_armature: Optional[bpy.types.Object] = None, external_bones: Optional[list[Bone]] = None, materials: Optional[list[bpy.types.Material]] = None, game: SollumzGame = SollumzGame.RDR1):
...
def create_model_obj(model_data: ModelData, materials: list[bpy.types.Material], name: str, bones: Optional[list[bpy.types.Bone]] = None):
    model_obj = create_blender_object(SollumType.DRAWABLE_MODEL, name, sollum_game_type=SollumzGame.RDR1)
...
def set_skinned_model_properties(drawable_obj: bpy.types.Object, drawable_xml: Drawable):
    ...
    for lod_level, models in get_model_xmls_by_lod(drawable_xml).items():
        if current_game == SollumzGame.RDR1 and isinstance(models, LodList):
...
def set_drawable_model_properties(model_props: DrawableModelProperties, model_xml: DrawableModel):
    model_props.render_mask = model_xml.render_mask if model_xml.render_mask is not None else 205
    model_props.flags = model_xml.flags if model_xml.flags is not None else 0

def create_drawable_armature(drawable_xml: Drawable, name: str):
    drawable_obj = create_armature_obj_from_skel(
        drawable_xml.skeleton, name, SollumType.VISUAL_DICTIONARY)
...
def create_drawable_empty(name: str, drawable_xml: Drawable):
    drawable_obj = create_empty_object(SollumType.VISUAL_DICTIONARY, name)
    set_drawable_properties(drawable_obj, drawable_xml)
