...
def import_wfd(filepath: str):
    import_settings = get_import_settings()

    name = get_filename(filepath)
    wfd_xml = WFD.from_xml_file(filepath)

    if import_settings.import_as_asset:
        return create_drawable_as_asset(wfd_xml, name, filepath)

    return create_drawable_obj(wfd_xml, filepath)
...
def create_drawable_armature(drawable_xml: Drawable, name: str):
    drawable_obj = create_armature_obj_from_skel(drawable_xml.skeleton, name, SollumType.FRAG_DRAWABLE)
...
def create_drawable_empty(name: str, drawable_xml: Drawable):
    drawable_obj = create_empty_object(SollumType.FRAG_DRAWABLE, name)
