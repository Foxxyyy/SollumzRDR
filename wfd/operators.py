...
from ..ydr.shader_materials import create_shader, create_tinted_shader_graph, is_tint_material, rdr1_shadermats
...
class SOLLUMZ_OT_create_drawable(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        drawable_obj = create_empty_object(SollumType.FRAG_DRAWABLE)
...
class SOLLUMZ_OT_convert_to_drawable(bpy.types.Operator):
    ...
    def convert_separate_drawables(
        ...
        sollum_game_type: SollumzGame = SollumzGame.RDR1
    ):
        for obj in selected_meshes:
            ...
            with context.temp_override(collection=obj.users_collection[0]):
                drawable_obj = convert_obj_to_visual_dict(obj, sollum_game_type)
    ...
    def convert_to_single_drawable(
        ...
        with context.temp_override(collection=target_coll):
            drawable_obj = convert_objs_to_single_visual_dict(selected_meshes, sollum_game_type)
...
class SOLLUMZ_OT_create_shader_material(SOLLUMZ_OT_base, bpy.types.Operator):
    ...
    def run(self, context):
        ...
        for obj in objs:
            shader = rdr1_shadermats[context.scene.shader_material_index].value
            try:
...
class SOLLUMZ_OT_move_shader_up(bpy.types.Operator):
    ...
    def poll(self, context):
        ...
        if aobj is None or aobj.sollum_type != SollumType.VISUAL_DICTIONARY:
            return False
