...
from .cwxml.drawable import WVD, YDR, YDD
...
class SOLLUMZ_OT_import(bpy.types.Operator, ImportHelper, TimedOperator):
    ...
    filter_glob: bpy.props.StringProperty(
        default=f"*{YDR.file_extension};*{YDD.file_extension};*{YFT.file_extension};*{YBN.file_extension};*{YNV.file_extension};*{YCD.file_extension};*{YMAP.file_extension};*{WVD.file_extension};",
        options={"HIDDEN", "SKIP_SAVE"},
        maxlen=255,
    )
...
class SOLLUMZ_OT_export(bpy.types.Operator, TimedOperator):
    ...
    filter_glob: bpy.props.StringProperty(
        default=f"*{YDR.file_extension};*{YDD.file_extension};*{YFT.file_extension};*{YBN.file_extension};*{YCD.file_extension};*{YMAP.file_extension};*{WVD.file_extension};",
        options={"HIDDEN", "SKIP_SAVE"},
        maxlen=255,
    )
...
def sollumz_menu_func_import(self, context):
    self.layout.operator(SOLLUMZ_OT_import.bl_idname,
                         text=f"Codewalker XML({YDR.file_extension}, {YDD.file_extension}, {YFT.file_extension}, {YBN.file_extension}, {YCD.file_extension}, {WVD.file_extension})")
...
class SOLLUMZ_OT_debug_hierarchy(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in context.selected_objects:
            ...
            elif sollum_type == SollumType.VISUAL_DICTIONARY:
                self.fix_visual_dict(obj)
    ...
    def fix_visual_dict(self, obj: bpy.types.Object):
        for model in obj.children:
            if model.type != "MESH":
                continue

            model.sollum_type = SollumType.VISUAL_DICTIONARY
